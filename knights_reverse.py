...
def create_log(path,pop_count, step, population):
    pop_dict = {}
    for pop in population:
        value = pop_dict.get(pop[-1])
        if value:
            value.append(pop)
        else:
            pop_dict[pop[-1]] = [pop]
    output = []
    output.append("step " + str(step) + ":\n")
    for key in pop_dict:
        output.append(str(key) + ":")
        output.append(str(len(pop_dict[key])/pop_count*100)+"%"+"\n")
    #Writing answer to a file
    for pop in population:
        output.append(str(pop) + "\n")
        output.append(generate_fenotip(pop[:len(pop)-1]))
    logger_file.print_to_file(output, path + "\\step"  + str(step))
...
def genetic_algorithm(round,size,pop_count,parent_count,mutation_rate,step_count):
    #Logging  operations
    path = "examples\\reverse\\percent\\percent" + str(round) 
    logger_file.create_dir(path)
    ...
    for step in range(step_count):
        ...
        #Survivel selection        
        #Create subgroups based on fitness
        pop_dict = {}
        for pop in population:
            value = pop_dict.get(pop[-1])
            if value:
                ...
                pop_dict[pop[-1]] = [pop]
        #Checking for domination - more than 30% of a group
        dominated = False
        for key in pop_dict:
            if len(pop_dict[key])/pop_count > 0.3:
                dominated = True
        if dominated:
            population = reverse_selection(pop_count, pop_dict,population)
        else:
            population = tournament(size, pop_count, population)
        #Creating output files for each generation
        create_log(path,pop_count,step,population)
...
for i in range(5):
    answers = genetic_algorithm(i,size,pop_count,parent_count,mutation_rate,step_count)